...
def non_max_suppression(prediction, nc, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                        labels=(), max_det=300):
    """Runs Non-Maximum Suppression (NMS) on inference results

    Returns:
         list of detections, on (n,6) tensor per image [xyxy, conf, cls, other]
    """

    # nc = prediction.shape[2] - 5  # number of classes
    other = prediction.shape[2] - (5+nc) 
    ...
    t = time.time()
    output = [torch.zeros((0, 6+other), device=prediction.device)] * prediction.shape[0]
